...
int main(int argc, char *argv[]) {
    ...
    if (argc != 11) {
        fprintf(stderr, "Uso: %s Valor HMat1 WMat1 HMat2 WMat2 NThreads ArqMat1 ArqMat2 ArqRes1 ArqRes2\n", argv[0]);
        return 1;
    }
    ...
    const char *file_A_in = argv[7];
    const char *file_B_in = argv[8];
    const char *file_res1_out = argv[9];
    const char *file_res2_out = argv[10];
